...
ПРАВИЛА:
1. "Группы" — сегмент (премиум, постоянный клиент, новый, маркетплейс, «событие <месяц>» и т.д.)
...
5. "Теги" — одна строка с хэштегами через пробел: #деньрождения #др_март #vip #доволен (не массив JSON).
6. "Саммари" — 2–4 предложения о МОТИВАЦИИ покупки (intent), не о профиле.
   Обязательно: событие + КОГДА (день/месяц), intent, предпочтения (цветы, доставка, оплата) из переписки.
   Запрещено голое «день рождения» без месяца; если месяц неизвестен — так и напиши.
7. "Саммари клиента" — 4–7 предложений: профиль, календарь событий с датами/месяцами,
   предпочтения по цветам/доставке/оплате, тон переписки. Не рекомендация оператору.
8. "Рекомендация" — 2–3 предложения оператору: ЧТО предложить, КОГДА (окно относительно события/месяца,
   не только «за 3 дня»), КАК связаться. Первый заказ за несколько дней до праздника цветов в РФ
   (14/23 февраля, 8 марта, 1 сентября, День матери, День учителя, Новый год) → этот повод + касание на следующий год.
   Без среднего чека не пиши «привычный средний чек» — дай типовой оффер под праздник.
...
11. "Дата рождения" — только если явно указана (ДД.ММ.ГГГГ или ДД.ММ)
...
ВАЖНО:
- Главный источник паттернов — orders_sample + order_marketing_patterns (даты/месяцы/повторы лет/поводы).
- Учитывай all_comments: способ оплаты, окна доставки, явные даты ДР.
- Из переписки извлекай: даты ДР/праздников, любимые цветы, адрес/время доставки, жалобы.
...
class MessengerEnrichmentService:
    ...
    async def _enrich_with_ai(
        ...
        payload = {
            ...
            "orders_sample": (row.get("_orders_context") or [])[:20],
        }
        from app.services.segmentation import SegmentationService

        patterns = SegmentationService.build_order_marketing_patterns(row)
        if patterns:
            payload["order_marketing_patterns"] = patterns
        comments = collect_client_comments(row)
        if comments:
            payload["all_comments"] = comments[:3000]
        user_prompt = (
