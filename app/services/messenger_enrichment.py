...
8. "Рекомендация" — маркетинговый бриф: Касание (даты+праздник) → Оффер+бюджет →
   Контекст (когда купил, цель, чек vs средний) → Канал. Первый заказ: блок «по похожим».
...
class MessengerEnrichmentService:
    ...
    async def _enrich_with_ai(
        ...
        user_prompt = (
            ...
            "«Рекомендация» — маркетинговый бриф: Касание (даты+праздник), Оффер, "
            "Контекст (когда/зачем, чек vs средний), для 1-го заказа — «по похожим», Канал. "
            'Ответ — JSON {"results": [...]}.\n\n'
