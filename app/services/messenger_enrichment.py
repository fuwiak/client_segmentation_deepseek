...
6. "Саммари" — 2–4 предложения о МОТИВАЦИИ покупки (intent), не о профиле.
   Событие + КОГДА, intent, предпочтения из переписки — связным текстом.
7. "Саммари клиента" — 4–7 предложений: имя, профиль, календарь событий, предпочтения, тон переписки.
   Не рекомендация оператору.
8. "Рекомендация" — 2–4 предложения живым языком: когда писать + праздник, оффер и бюджет,
   зачем покупал, чек vs средний, канал. Без ярлыков Касание/Оффер. Первый заказ: про похожих.
...
class MessengerEnrichmentService:
    ...
    async def _enrich_with_ai(
        ...
        user_prompt = (
            ...
            "«Рекомендация» — живым языком (когда писать, что предложить, зачем покупал, "
            "чек vs средний, канал), без ярлыков Касание/Оффер/Контекст. "
            'Ответ — JSON {"results": [...]}.\n\n'
