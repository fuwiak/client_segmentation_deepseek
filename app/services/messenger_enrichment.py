...
8. "Рекомендация" — 2–3 предложения: ЧТО + имя + КОГДА (окно) + КАК. Первый заказ рядом
   с праздником цветов в РФ → этот повод + касание на следующий год. Без «привычный средний чек».
...
class MessengerEnrichmentService:
    ...
    async def _enrich_with_ai(
        ...
        user_prompt = (
            ...
            "«Саммари клиента», «Саммари» и «Рекомендация» — narrative в стиле few-shot "
            "из system prompt, без дампа полей CRM. "
            'Ответ — JSON {"results": [...]}.\n\n'
